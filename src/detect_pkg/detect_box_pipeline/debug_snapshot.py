"""Per-request RGB-D, instance-mask and point-cloud debug snapshots."""
...
class DebugSnapshotWriter:
    """Persist only the annotated color image and masked ROI cloud."""
    ...
    def save(
        ...
        """Save the two requested debug artifacts for one request."""
        ...
        if detection is not None:
            mask = self._validated_mask(detection.mask, color.shape[:2])
            annotated = self._overlay_mask(color, mask)
            self._draw_detection(annotated, detection)
            self._write_image(
                snapshot_dir / "color_with_yoloe_mask.png",
                annotated,
            )

        if point_cloud is not None:
            self._write_ply(
                snapshot_dir / "mask_roi_cloud.ply",
                np.asarray(point_cloud.points_camera_m, dtype=np.float32),
    ...
    @staticmethod
    def _validated_mask(mask: Any, image_shape: tuple[int, int]) -> np.ndarray:
        result = np.asarray(mask, dtype=bool)
        if result.shape != image_shape:
            raise ValueError(
                f"Instance mask must have shape {image_shape}, got {result.shape}"
            )
        return result
    ...
    def _overlay_mask(image: np.ndarray, mask: np.ndarray) -> np.ndarray:
        ...
        if not np.any(mask):
            return overlay
        tint_bgr = np.array([0, 200, 255], dtype=np.float32)
        source = overlay[mask].astype(np.float32)
        overlay[mask] = np.clip(
            0.55 * source + 0.45 * tint_bgr,
            0.0,
            255.0,
        ).astype(np.uint8)
        return overlay
